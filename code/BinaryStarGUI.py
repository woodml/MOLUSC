...
class GUI(tk.Frame):
    ...
    def create_widgets(self):
        ...
        self.__jitter_check = tk.BooleanVar()
        ...
        self.__jitter_str = tk.StringVar()
        ...
        # Grid
        # checkbox row
        checkbox_frame.grid(column=0, row=1, columnspan=4)
        ao_box.grid(column=0, row=0)
        rv_box.grid(column=1, row=0)
        jitter_box.grid(column=2, row=0)
        ...
        self.__rv_file_box.grid(column=1, row=3)
        resolution_label.grid(column=2, row=3)
        self.__resolution_box.grid(column=3, row=3)
        # jitter row
        jitter_label.grid(column=0, row=4, stick='e')
        self.__jitter_entry_box.grid(column=1, row=4)
    ...
    def activate_jitter(self):
        if self.__jitter_check.get() == 1:  # whenever checked
            self.__jitter_entry_box.config(state=tk.NORMAL)
            self.__rv_file_box.config(state=tk.NORMAL)
        elif self.__jitter_check.get() == 0:  # whenever unchecked
            self.__jitter_entry_box.config(state=tk.DISABLED)
            self.__rv_file_box.config(state=tk.DISABLED)
    ...
    def check_completeness(self):
        ...
        if self.__jitter_check.get() and self.__rv_file.get():
            if self.__jitter_str.get() == '':
                self.__jitter_entry_box.config(bg='lightcoral')
                allow_run = False
                message = message +'- No stellar jitter value has been given.\n'
        elif self.__jitter_check.get() and not self.__rv_file.get():
            self.__jitter_entry_box.config(bg='lightcoral')
            message = message + '- The jitter cannot be calculated without RV.\n'
            allow_run = False
        # Coordinates
    ...
    def get_jitter(self):
        if self.__jitter_check.get():
            string = self.__jitter_str.get()
            try:
                jitt = float(string)
                return jitt
            except:
...
class Application:
    ...
    def run(self):
        ...
        if self.rv_filename:
            if self.star_jitter != -1:
                # Run RV with Jitter
                self.print_out('Analyzing RV and Jitter...')
                rv = RV(self.rv_filename, self.resolution, comps, self.star_mass, self.star_age, self.star_jitter, self.added_jitter, rv_floor=self.rv_floor, extra_output=self.extra_output)
                # Read in the RV file
                failure = self.error_check(rv.read_in_rv())
                if failure: return
                if self.extra_output: self.print_out('RV Measurements Loaded')
                rv.analyze_rv()  # The RV analysis must still be done, but the reject list will be ignored, in favor of the jitter reject list
                self.jitter_reject_list = rv.analyze_jitter()
    ...
    def get_inputs(self):
        ...
        self.star_jitter = self.gui.get_jitter()
    ...
    def parse_input(self):
        ...
        self.star_jitter = args.star_jitter
        # Output Options
